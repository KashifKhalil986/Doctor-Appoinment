...
const Doctor = () => {
  ...
  console.log("doc speciality", doctors?.map(d => d.speciality))
  const [filterDoc, setFilterDoc] = useState([])
  const[showFilter,setShowFilter] = useState(false)
  const navigate = useNavigate()
  const applyFilter = () => {
    if (speciality) {
      setFilterDoc(doctors.filter(doc => doc.speciality.replace(/\s+/g, '-').toLowerCase() === speciality.toLowerCase()))
    }
    else {
  ...
  return (
    <>
      <div className="">
       ...
       <button onClick={()=>setShowFilter(prev=>!prev)} className={`py-1 px-3 border rounded text-sm transition-all sm:hidden ${showFilter ? 'text-white bg-[#5f6FFF]' : ''}`}>Filters</button>
